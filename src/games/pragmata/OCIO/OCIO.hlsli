...
// Scene mid gray at the input of the tone curve. The game's own pre tone map
// curve runs before this in the PostProcess pass and is not gated on the tone
// mapper, so both tone mappers see the same signal here and place it the same
// way.
...
// 0.1024 / 0.1000 / 0.0995 for the three `ACES Mid Gray` targets, because the
...
float3 ApplyToneMapEncodePQ(float3 untonemapped_ap1, float cbuffer_peak_nits, float cbuffer_diffuse_white_nits, float2 uv) {
  ...
  if (TONE_MAP_TYPE != PRAGMATA_TONE_MAP_TYPE_PSYCHOV22) {
  ...
  if (TONE_MAP_TYPE == PRAGMATA_TONE_MAP_TYPE_PSYCHOV22) {
    ...
    // toward the interior, a shift of 0.029 in BT.2020 rg chromaticity.
    //
